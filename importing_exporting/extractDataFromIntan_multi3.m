function extractDataFromIntan_multi3(path)


path = [path '/'];
allFilesInFolder = dir;

files = [];
targetExtension = 'rhd';
idxRHDFiles = 0;
for idxFile = 1:length(allFilesInFolder)
    fileName = allFilesInFolder(idxFile).name;
    if length(fileName) > 3 
            extension = fileName(end-2:end);
            if strcmp(extension, targetExtension)
                idxRHDFiles = idxRHDFiles + 1;
                files{idxRHDFiles} = fileName;
            end
    end
end
