...
for i=1:length(tetrodes)
    ...
    th=th_tetrodes(i);
    ts=Timestamps;
    filename_tetrode = sprintf('tetrode%d_spikesK.mat', tetrode);
    load(filename_tetrode)
    clear cluster_index; clear clustersToKeep; clear pc; clear tetrodes_clusters;
    spike = double(spikes.waveforms);
    ...
    while L<=0
        if ~fsmem_alg
            [clusters,k,pc,L,D] = SortSpikes(spike,trainingSetSize,pcc,cth,th,ltol,calc_index,isTetrode); %Sort step
        else
            %Prepare test set, to be kept unchanged across multiple runs of the smem algorithm
            if isTetrode
                testSet=randperm(size(spike,1));
                testSet=testSet(1:round(size(spike,1)*testSetSize));
            else
                testSet=randperm(size(spike,1));
                testSet=testSet(1:round(size(spike,1)*testSetSize));
            end
            ...
            if choosePCA
                [clusters,k,pc,L,D,BIC,cont] = cluster_fsmem_PCA(spike,trainingSetSize,pcc,cth,th,ltol,calc_index,isTetrode,testSet);
                
            elseif chooseICA
                [clusters,k,pc,L,D,BIC,cont,ica2plot] = cluster_fsmem_ICA(spike,trainingSetSize,pcc,cth,th,ltol,calc_index,isTetrode,testSet,n_ica);

            else chooseWavelet
                [clusters,k,pc,L,D,BIC,cont] = SortSpikes_fsmem_WAV(spike,trainingSetSize,n_coeff,cth,th,ltol,calc_index,isTetrode,testSet);
            end
        end
...
%         IsDis(1)=[];
        ...
        stringa='False negative+positive indexes | Spikes in violation zone:';
        for j=1:length(cont)
            stringa=sprintf('%s\n Cluster %d: %d | %d/%d',stringa,j,cont(j), violating(j), tot_spikes(j));
        end
