        ...
        for idxUnit = 1:length(shankNowarp(idxShank).cell)
            for idxOdor = 1:odors
                ...
                if pvals < 0.005 && (mean(a{2}) == mean(a{1}))
                    exp(idxExperiment).shankNowarp(idxShank).cell(idxUnit).odor(idxOdor).pValue300ms = 1;
                end
                meanBsl = mean(appBsl);
                stdBsl = std(appBsl);
                ...
                if pvals < 0.005 && (mean(a{2}) == mean(a{1}))
                    exp(idxExperiment).shankNowarp(idxShank).cell(idxUnit).odor(idxOdor).pValue1000ms = 1;
                end
                meanBsl = mean(appBsl);
                stdBsl = std(appBsl);
                goodTrialsExc = appRsp > (meanBsl + 2*stdBsl);
                ...
                if pvals < 0.005 && (mean(a{2}) == mean(a{1}))
                    exp(idxExperiment).shankNowarp(idxShank).cell(idxUnit).odor(idxOdor).pValue2000ms = 1;
                end
                meanBsl = mean(appBsl);
                stdBsl = std(appBsl);
                goodTrialsExc = appRsp > (meanBsl + 2*stdBsl);
