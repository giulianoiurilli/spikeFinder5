...
mkdir(toFolder)
filename = sprintf('val_analysis_classification-apCx.mat');
        ...
        for s=1:length(shank(sha).spike_matrix)
            for k=1:10 %odors
                ...
                for whichBin = 1: response_window./bin_size %pre/bin_size;
                    populationPatternTrial{whichBin}(unit,:,k) = app_delta_response(whichBin,:); %app_response(whichBin,:);%app_base1(whichBin,:); %;%app_psth(whichBin,:)
                end
                %end
            end
            unit = unit + 1;
...
stringa_fig=sprintf('Tuning functions - best.eps');
saveas(h,fullfile(toFolder,stringa_fig),'epsc')


save(fileSave, 'pop_activity_full', 'populationPatternTrial', 'best_pop_activity_full', 'best_populationPatternTrial', 'ls', 'd_prime', 'dprime_rank', 'bsl_mean', 'bsl_var', 'rsp_mean', 'rsp_var')
...
shadedErrorBar(1:10, meanAccuracy_svm, sdAccuracy_svm, 'r');
set(gca,'FontName','Arial','Fontsize',12, 'FontWeight', 'normal');
set(h,'color','white', 'PaperPositionMode', 'auto');
stringa_fig=sprintf('c_svm linear discrimination -%s .eps', classes);
saveas(h,fullfile(toFolder,stringa_fig),'epsc')
