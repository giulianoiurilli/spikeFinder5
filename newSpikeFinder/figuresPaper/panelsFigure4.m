...
%% figure
figure
plot(mean(performancePcx15,1), 'color', pcxC, 'linewidth', 2);
hold on
plot(mean(performanceCoa15,1), 'color', coaC, 'linewidth', 2);
...
option = [];
option.units = 'all';
option.repetitions = 1000;
option.grouping = [ones(1,5) 2*ones(1,5)];
[performanceValenceCoaAll, cm, weigthsValenceCoa_2] = perform_linear_svm_decoding(coaAA.esp, odorsRearrangedCoa,option);
[performanceValencePcxAll, cm, weigthsValencePcx_2] = perform_linear_svm_decoding(pcxAA.esp, odorsRearrangedPcx,option);
%%
% odors = [1 2 3 11 12 13 4 5 6 8 9 10 7 14 15];
option = [];
option.units = 'incrementing_by_10';
...
for idxUnit = 1:totalResponsiveSUAPcx
    ...
    sua_CM_pcx(idxUnit) = miApp;
end

%%
[fiPcx, xiPcx] = ksdensity(sua_MI_pcx);
[fiCoa, xiCoa] = ksdensity(sua_MI_coa);
all_xiPcx = nan(100,length(fiPcx));
all_xiCoa = nan(100,length(fiCoa));
for k = 1:100
    idxPcx = randsample(length(sua_MI_pcx), 50);
    idxCoa = randsample(length(sua_MI_coa), 50);
    [fiPcx, xiPcx] = ksdensity(sua_MI_pcx(idxPcx));
    [fiCoa, xiCoa] = ksdensity(sua_MI_coa(idxCoa));
    all_xiPcx(k,:) = fiPcx;
    all_xiCoa(k,:) = fiCoa;
end
...
figure
set(gcf,'color','white', 'PaperPositionMode', 'auto');
shadedErrorBar(xiPcx, mean(all_xiPcx), std(all_xiPcx)./10, 'b')
hold on
shadedErrorBar(xiCoa, mean(all_xiCoa), std(all_xiCoa)./10,  'r')
...
%% figure
figure
plot(mean(performancePcx15Sorted,1), 'color', pcxC, 'linewidth', 2);
hold on
plot(mean(performanceCoa15Sorted,1), 'color', coaC, 'linewidth', 2);
...
%%
[fiPcx, xiPcx] = ksdensity(sua_MI_pcxChemical);
[fiCoa, xiCoa] = ksdensity(sua_MI_coaChemical);
reps = 500;
all_xiPcx = nan(reps,length(fiPcx));
all_xiCoa = nan(reps,length(fiCoa));
for k = 1:reps
    idxPcx = randsample(length(sua_MI_pcxChemical), 20);
    idxCoa = randsample(length(sua_MI_coaChemical), 20);
    [fiPcx, xiPcx] = ksdensity(sua_MI_pcxChemical(idxPcx));
    [fiCoa, xiCoa] = ksdensity(sua_MI_coaChemical(idxCoa));
    all_xiPcx(k,:) = fiPcx;
    all_xiCoa(k,:) = fiCoa;
end
...
figure
set(gcf,'color','white', 'PaperPositionMode', 'auto');
shadedErrorBar(xiPcx, mean(all_xiPcx), std(all_xiPcx)./sqrt(reps), 'b')
hold on
shadedErrorBar(xiCoa, mean(all_xiCoa), std(all_xiCoa)./sqrt(reps),  'r')
xlabel('information (bits)')
ylabel('p.d.f.')
axis tight
set(gca, 'box', 'off', 'tickDir', 'out', 'fontname', 'helvetica', 'fontsize', 14)
...
%%
[fiPcx, xiPcx] = ksdensity(sua_MI_pcxValence);
[fiCoa, xiCoa] = ksdensity(sua_MI_coaValence);
figure
set(gcf,'color','white', 'PaperPositionMode', 'auto');
plot(xiPcx, fiPcx, 'linewidth', 1, 'color', pcxC)
hold on
plot(xiCoa, fiCoa, 'linewidth', 1, 'color', coaC)
xlabel('information (bits)')
ylabel('p.d.f.')
axis tight
set(gca, 'box', 'off', 'tickDir', 'out', 'fontname', 'helvetica', 'fontsize', 14)
...
all_xiCoa = nan(100,length(fiCoa));
for k = 1:100
    idxPcx = randsample(length(sua_MI_pcxValence), 50);
    idxCoa = randsample(length(sua_MI_coaValence), 50);
    [fiPcx, xiPcx] = ksdensity(sua_MI_pcxValence(idxPcx));
    [fiCoa, xiCoa] = ksdensity(sua_MI_coaValence(idxCoa));
    all_xiPcx(k,:) = fiPcx;
    all_xiCoa(k,:) = fiCoa;
end
...
option = [];
option.units = 'remove';
option.sorting_vector = sua_performance_coaValence;
option.repetitions = 500;
option.grouping = [ones(1,5) 2*ones(1,5)];
performanceValenceCoaRemoved = perform_linear_svm_decoding(coaAA.esp, odorsRearrangedCoa,option);

option = [];
option.units = 'remove';
option.sorting_vector = sua_performance_pcxValence;
option.repetitions = 500;
option.grouping = [ones(1,5) 2*ones(1,5)];
performanceValencePcxRemoved = perform_linear_svm_decoding(pcxAA.esp, odorsRearrangedPcx,option);
%%
...
%%
[tuningCurvesCoa15, tuningCurvesSigCoa15, tuningCurvesCoaAuRoc15, tuningCurvesCoaAuRocSig15] = findTuningCurves(coa15.esp, 1:15, 0);
[tuningCurvesPcx15, tuningCurvesSigPcx15, tuningCurvesPcxAuRoc15, tuningCurvesPcxAuRocSig15] = findTuningCurves(pcx15.esp, 1:15, 0);
%%
