        ...
        switch floor(opt.histOpt)
            ...
            case 1
                ...
                if opt.histOpt == 1.1
                    ...
                    [dummy,x] = my_histogram(currentData); %#ok<ASGLU>
                    if length(x) == 1
            ...
            case 2
                ...
                [xHist,yHist] = my_histogram(currentData,opt.divFactor,0);
            case 99
...
if opt.addSpread
    if isHistogram
        ...
        try
            ...
            set(sh{1},'color',cell2mat(opt.color));
            %set(sh{1},'color',opt.color);
    ...
    switch opt.xyOri
        case 'normal'
            if any(opt.showMM==[1,2])
                mh = plot(ah,opt.xValues+xOffset,m,'+r','Color',cell2mat(opt.color),'MarkerSize',12);
            end
            ...
            if opt.showMM == 6
                mdh(1,:) = plot(ah,squeeze(md(:,1,2:3))',repmat(md(:,1,1)',2,1),'color',cell2mat(opt.color),'lineWidth',2);%,'lineStyle','--');
                mdh(2,:) = plot(ah,squeeze(md(:,2,2:3))',repmat(md(:,2,1)',2,1),'color',cell2mat(opt.color),'lineWidth',1);%,'lineStyle','--');
                mdh(3,:) = plot(ah,squeeze(md(:,3,2:3))',repmat(md(:,3,1)',2,1),'color',cell2mat(opt.color),'lineWidth',1);%,'lineStyle','--');
            end
        case 'flipped'
            if any(opt.showMM==[1,2])
                mh = plot(ah,m,opt.xValues+xOffset,'+r','Color',cell2mat(opt.color),'MarkerSize',12);
            end
            if any(opt.showMM==[1,3])
            ...
            if opt.showMM == 6
                mdh(1,:) = plot(ah,repmat(md(:,1,1)',2,1),squeeze(md(:,1,2:3))','color',cell2mat(opt.color),'lineWidth',2);%,'lineStyle','--');
                mdh(2,:) = plot(ah,repmat(md(:,2,1)',2,1),squeeze(md(:,2,2:3))','color',cell2mat(opt.color),'lineWidth',1);%,'lineStyle','--');
                mdh(3,:) = plot(ah,repmat(md(:,3,1)',2,1),squeeze(md(:,3,2:3))','color',cell2mat(opt.color),'lineWidth',1);%,'lineStyle','--');
            end
    end
end
