...
handles.bar = bar(varargin{1}); % standard implementation of bar fn
hold on
hBar = handles.bar;
...
x = bsxfun(@plus, hBar(col).XData, [hBar(col).XOffset]');
        ...
        hErrorbar(col) = errorbar(mean(x,1), values(xOrder,col), lowerErrors(xOrder,col), upperErrors(xOrder, col), colorToUse);
    ...
    hErrorbar = errorbar(mean(x,1), values, lowerErrors, upperErrors, colorToUse);
