...
for iData = 1:nData
    ...
    if ~isempty(currentData)
        ...
        if opt.showMM > 0
            ...
            iqrOversqrtN(iData) = (prctile(currentData(:,2),75)-prctile(currentData(:,2),25))   /sqrt(sum(isfinite(currentData(:,2))));
        end
    end % test isempty
end
            ...
            switch opt.xyOri
                case 'normal'
                    ph(iData,iCategory) = plot(ah,data(currentIdx,1),...
                        ...
                        'MarkerSize',8,...
                        'MarkerEdgeColor',plotColors{iData,iCategory},...
    ...
    switch opt.xyOri
        case 'normal'
            ...
            if any(opt.showMM==[1,3])
                mdh = plot(ah,opt.xValues,md,'sk','MarkerSize',12);
            end
            if opt.showMM == 4
                mh = plot(ah,opt.xValues,m,'+r','Color','r','MarkerSize',12);
            ...
            if opt.showMM == 6
                mh = plot(ah,opt.xValues,md,'sb','Color','b','MarkerSize',12);
                mdh = myErrorbar(ah,opt.xValues,md,1.57*iqrOversqrtN(:));
            end
        case 'flipped'
            if any(opt.showMM==[1,2])
            ...
            if any(opt.showMM==[1,3])
                mdh = plot(ah,md,opt.xValues,'sk','MarkerSize',12);
            end
            if opt.showMM == 4
                mh = plot(ah,m,opt.xValues,'+r','Color','r','MarkerSize',12);
            ...
            if opt.showMM == 5
                mh = plot(ah,m,opt.xValues,'+r','Color','b','MarkerSize',12);
                mdh = myErrorbar(ah,m,opt.xValues,[sd,NaN(size(sd))]);
            end
            if opt.showMM == 6
                mh = plot(ah,md,opt.xValues,'sb','Color','b','MarkerSize',12);
                mdh = myErrorbar(ah,md,opt.xValues,[1.57*iqrOversqrtN(:),NaN(size(sd))]);
            end
    end
end
