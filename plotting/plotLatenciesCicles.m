        ...
        for idxUnit = 1:length(shank(idxShank).spiketimesUnit)
            for idxOdor = 1:odors
                if shank(idxShank).cell(idxUnit).odor(idxOdor).meanPeakExcitation(idxCycle) == 1;
                    ...
                    excitatoryResponses(i,:) = mean(shank(idxShank).cell(idxUnit).odor(k).sdf_trialRadMs);
...
%                 end
    ...
    excitatoryResponses1 = excitatoryResponses(:, 1:(2+idxCycle) * cycleLength);
...
%     inhibitoryResponses1(:,cycleLength:cycleLength + idxCycle) = max(max(inhibitoryResponses1));
    ...
    stringa = sprintf('Excitatory responses tile the respiratory cycle %d', idxCycle);
