function spikes = new_detect( data, spikes )
...
noise = median(abs(data))/0.6745;


spikes.info.detect.stds = noise;


thresh = - params.thresh * noise;                                             %thr for detection is based on detected settings.
...
% find all threshold crossings
    ...
    % get crossings on all channels 
    crossings = [];
    channel = [];
    for k = 1:num_channels
        crossings = [crossings find( data(1:end-1,k) > thresh(k) &  data(2:end,k) <= thresh(k) )' ];
    ...
    channel(  crossings > size(data,1) - samples_after   ) = [];    crossings(  crossings > size(data,1) - samples_after ) = [];

    % update spiketimes and waveforms
    spikes.spiketimes   =  [spikes.spiketimes crossings / params.Fs];
    spikes.trials       =  [spikes.trials pre_trials + (1 * ones( [1 length(crossings)] ))];
    ...
    spikes1.waveforms    =  [spikes.waveforms; w ];

    spikes.info.detect.dur = size( data, 1) / params.Fs;


clear data
% save everything
spikes1.waveforms = single(spikes1.waveforms);
...
divisor = repmat( spikes.info.detect.thresh, [size(spikes1.waveforms,1) 1] );  
[junk,  spikes.info.detect.event_channel] = max( squeeze( min( spikes1.waveforms(:,jitter_range,:), [], 2 ) )./divisor, [], 2 );
