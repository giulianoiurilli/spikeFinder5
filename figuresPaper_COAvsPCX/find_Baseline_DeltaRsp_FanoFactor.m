function [Bsl, BslAll, DeltaRspMean, DeltaRsp, rspMean, rspVar, ff, cv, auRoc, varG, rspFractionExc, rspFractionInh, significance] = find_Baseline_DeltaRsp_FanoFactor(esp, odors, window)
...
DeltaRsp = zeros(c, 10, length(odors)); 
            ...
            if esp(idxExp).shankNowarp(idxShank).cell(idxUnit).good == 1
                ...
                for idxOdor = odors
                    ...
                    if window  == 300
                        ...
                        varG(c,idxO) = partNeuralVariance(R1000ms(:, idxO));
                        rspFractionExc(c,idxO) = esp(idxExp).shankNowarp(idxShank).cell(idxUnit).odor(idxOdor).responseFractionExc;
                        rspFractionInh(c,idxO) = esp(idxExp).shankNowarp(idxShank).cell(idxUnit).odor(idxOdor).responseFractionInh;
                    end
                end
