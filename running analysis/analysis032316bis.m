...
blankFigure([leftX-10*yMargin rightX+10*xMargin bottomY-10*bottomMargin topY+10*topMargin]);
hold on
plot(ls1000Coa, info1000Coa, 'ok', 'markerfacecolor',coaC,'markersize', 4,  'markeredgecolor', coaC)
hold on
plot(ls1000Pcx, info1000Pcx, 'ok', 'markerfacecolor', pcxC,'markersize', 4,  'markeredgecolor', pcxC)
...
figure
set(gcf,'color','white', 'PaperPositionMode', 'auto');
dataToPlot = {info1000Coa,info1000Pcx};
catIdx = [ones(length(info1000Coa),1); zeros(length(info1000Pcx),1)];
colori = {pcxC,coaC};
plotSpread(dataToPlot,'categoryIdx',catIdx,'categoryColors', colori, 'showMM', 0, 'xyOri', 'flipped')
set(gca, 'YColor', 'w', 'box', 'off')
xlabel('Odor Predictability (bits)')
%xlim([0 1])
set(gca, 'TickDir', 'out')
%%
R1000Coa  = [selectivityCoa15.LI1000';selectivityCoaAA.LI1000'];
