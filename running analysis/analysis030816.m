...
for idxTrial = 1:n_trials
    ...
    idxU = 0;
    for idxShank = 1:4
        n_units = size(Data.Spiketimes.Experiment(idxExp).Shank(idxShank).spiketimes, 3);
        for idxUnit = 1:n_units
            idxU = idxU + 1;
            cellID= idxUnit;
            spikeTimes{idxU} = Data.Spiketimes.Experiment(idxExp).Shank(idxShank).spiketimes{idxTrial, idxOdor, idxUnit};
            nst{idxU} = nspikeTrain(spikeTimes{idxU});
            nst{idxU}.setName(num2str(cellID));
    ...
    neuronsColl{idxTrial} = nstColl(nst);
    covariatesColl{idxTrial} = CovColl({baseline, sniff, stim, sniffAndStimulus});
    trial{idxTrial} = Trial(neuronsColl{idxTrial}, covariatesColl{idxTrial});
