...
function spikes = default_params(Fs, varargin )
% 
%
...
%
    ...
    spikes.params.thresh = -8;            % for 'auto', set threshold above background noise
    spikes.params.maxthresh = -20;         % for 'auto, set threshold for large artifacts
 %  spikes.params.thresh =    [-5.3897 -5.0999 -5.1816 -5.5274]; % for 'manual', set specific thresholds for each channel
    spikes.params.window_size  = 1.6;       % ms, width of a spike
    ...
    % sorting parameters
    spikes.params.refractory_period = 2;  % ms, refractory period (for calculation refractory period violations)
    spikes.params.max_jitter = 0.6;         % ms, width of window used to detect peak after threshold crossing
    spikes.params.agg_cutoff = .001;         %  higher = less aggregation, lower = more aggregation
    ...
    spikes.params.display.show_isi = 1;                 % 0 = show autocorrelation by default, 1 = show ISI histogram
